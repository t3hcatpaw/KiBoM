...
class BomPref:
    ...
    def __init__(self):
        ...
        self.mergeDNC = True  # Merge rows for do-not-change parts
    ...
    def Read(self, file, verbose=False):
        ...
        if self.SECTION_GENERAL in cf.sections():
            ...
            self.mergeDNC = self.checkOption(cf, self.OPT_MERGE_DNC, default=True)
    ...
    def Write(self, file):
        ...
        self.addOption(cf, self.OPT_MERGE_DNC, self.mergeDNC, comment="If '{opt}' option is set to 1, rows that are not to be changed on the PCB will be merged to equivalent parts in the BoM file".format(opt=self.OPT_MERGE_DNC))
